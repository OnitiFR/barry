...
func NewAlertSender(configPath string, log *Log) (*AlertSender, error) {
	...
	if len(list) == 0 {
		log.Warningf(MsgGlob, "no alert script found (no *.sh files in '%s') so you're currently blind to background failures", scriptsPath)
	}
...
func (sender *AlertSender) Send(alert *Alert) error {
	...
	for _, script := range alertScripts {
		...
		if cmdOut, err := cmd.CombinedOutput(); err != nil {
			sender.log.Errorf(MsgGlob, "error running alert script '%s': %s, output: %s", script, err, cmdOut)
			scriptsWithError = append(scriptsWithError, script)
		} else {
			sender.log.Infof(MsgGlob, "alert run was successfull '%s'", script)
		}
	}
